...
// Filter articles search function 
function filterArticles() {
	// Takes users search input and converts to lowercase
    const input = document.getElementById('search-input').value.toLowerCase();
	// Selects all the articles inside the article container 
    const articleBlocks = document.querySelectorAll('#article-container .page-block');

    // Loops through each article
    articleBlocks.forEach(block => {
	// Gets the text content of the article and converts to lowercase
        const text = block.textContent.toLowerCase();
	// If the select article includes characters from the search input display it
        if (text.includes(input)) {
            block.style.display = '';			
        } 
	// Otherwise hide the element from the page
		else {
